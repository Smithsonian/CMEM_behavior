...
test_colnames <- function(datasets, database_structure_doc = "data/datasetStandards/CCN Biomass and Elevation Synthesis Guidence 220331.csv") {
  ...
  for (i in 1:length(datasets)) {
    ...
    controlled <- database_structure %>%
      ...
      dplyr::select(heriarchy, attribute_name, data_type, units)
...
test_requirements <- function(datasets, database_structure_doc = "data/datasetStandards/CCN Biomass and Elevation Synthesis Guidence 220331.csv") {
  database_structure <- readr::read_csv(database_structure_doc, col_types = cols())
...
test_variables <- function(datasets, database_structure_doc = "data/datasetStandards/CCN Biomass and Elevation Synthesis Guidence 220331.csv") {
    ...
    if (length(factors_present) > 0) {
      for (j in 1:length(factors_present)) {
        ...
        check_these_variables <- unique(
          datasets[[i]] %>% dplyr::select(check_this_attribute) %>% 
...
# Need a function to reorder columns automatically
order_columns <- function(dataset, category, database_structure_doc = "data/datasetStandards/CCN Biomass and Elevation Synthesis Guidence 220331.csv") {
  ...
  sub_df <- database_structure %>% 
    filter(heriarchy == category)
  
  attributes_possible <- sub_df$attribute_name
  
  attributes_present <- names(dataset) 
  
  dataset_output <- dataset[,
                            attributes_possible[attributes_possible %in% attributes_present]
                            ]
